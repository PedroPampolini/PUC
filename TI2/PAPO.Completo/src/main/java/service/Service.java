...
public class Service {
	...
	public static String search(Request request) {
		...
		for(int i = 0; i < professores.size(); i++) {
			...
			/*----Acrescente materia no campo do professor----*/
			String materiaProfessor = new String();
			for(int j = 0; j < materias.size(); j++) {
				
				if(p.getId().equals(materias.get(j).getProfessor())) {
					List<String> materiaDoProfessor = professorDao.getProfessorMateria(materias.get(j).getProfessor());
					for(int k = 0; k < materiaDoProfessor.size();k++) {
						materiaProfessor += materiaDoProfessor.get(k) + ",";
					}
					
				}
			}
			professorJsonObj.put("materia", materiaProfessor);
	...
	public static boolean insertProfessor(Request request) {
		...
		boolean resp = pd.insert(professor);
	...
	public static String getProfessores(){
		...
		for(int i = 0; i < professores.size(); i++) {
			...
			for(int j = 0; j < materias.size(); j++) {
				...
				if(p.getId().equals(materias.get(j).getProfessor())) {
					List<String> materiaDoProfessor = pd.getProfessorMateria(materias.get(j).getProfessor());
					for(int k = 0; k < materiaDoProfessor.size();k++) {
						materiaProfessor += materiaDoProfessor.get(k) + ",";
					}
