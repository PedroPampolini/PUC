public class NeuralNetwork
{
    ...
    double bias;
    int maxAttempts;
    public NeuralNetwork(int[] layer, int epochs = 100, int randomSeed = -1, double learningRate = 0.001, double bias = 1, double tolerate = 0.001, int maxAttempts = 5 , Func<double, double>? activationFunction = null, Func<double, double>? dactivationFunction = null)
    {
        if(randomSeed == -1){
        ...
        for (int i = 0; i < layers.Length; i++)
            layers[i] = new Layer(layer[i], layer[i + 1], randomSeed:randomSeed, bias:bias,learningRate:learningRate, activationFunction:activationFunction, activationFunctionDerivative:dactivationFunction);
