...
public class Layer{
    ...
    public Layer(int numberOfInputs, int numberOfOutputs, int randomSeed = -1, double bias = 1, double learningRate = 0.001, Func<double, double>? activationFunction = null, Func<double, double>? activationFunctionDerivative = null){
        if(activationFunction == null)
            activationFunction = (x) => {return (1 / (1 + Math.Exp(-x)));};     //Default activation function, TANH
        if(activationFunctionDerivative == null)
            activationFunctionDerivative = (x) => {return (1 - (x * x));};      //Default derivative of the activation function, TANH
        if(randomSeed == -1){
    ...
    public void InitializeWeights(int randomSeed, double bias=1){
        Random gen = new Random(Seed: randomSeed);
        for (int i = 0; i < this.outputs.Length; i++){
