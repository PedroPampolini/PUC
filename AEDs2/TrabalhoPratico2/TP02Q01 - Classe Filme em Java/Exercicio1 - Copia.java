...
public class Exercicio1 {
   ...
   public static void main(String[] args) throws Exception {
      ...
      String[] listaMovies = new String[100]; // Array com todos os nomes dos filme
      Filme[] movies = new Filme[100]; // Array com todos os filmes da lista
      int index = 0;
      listaMovies[index] = MyIO.readLine();
      // MyIO.setCharset("UTF-8");
      while (!isFim(listaMovies[index++])) {
         listaMovies[index] = MyIO.readLine();
      }
      index = 0;
      while (listaMovies[index].charAt(0) != 'F') {// !isFim(listaMovies[index])) {
         // System.out.println(listaMovies[index]);
         movies[index] = new Filme(listaMovies[index]);
         movies[index].getAll();
         index++;
      }
      ...
      for (int i = 0; i < movies.length; i++) {
         if (movies[i] == null) {
            ...
            // MyIO.println("cu");
            MyIO.println(movies[i].toString());
         }
      }
