...
classe Filme. Então ordena os filmes a partir do atributo de situação utilizando o algorítmo merge sort
*/
...
public class TP03Q09 {
   ...
   public static int cmpFilmes(Filme f1, Filme f2) {
      int resp = 0;
      resp = f1.getDuracao() - f2.getDuracao();
      if (resp == 0) {
   ...
   public static Filme[] mergeSort(Filme[] catalogo){
      return mergeSort(catalogo, 0, catalogo.length - 1);
   }

   private static Filme[] mergeSort(Filme[] catalogo, int esq, int dir){
      count++;
      if(esq < dir){
         int mid = (dir + esq)/2;
         catalogo = mergeSort(catalogo, esq, mid); //Chama a recursão para a metade à esquerda
         catalogo = mergeSort(catalogo, mid+1, dir);  //Chama a recursão para a metade à direita
         catalogo = intercalar(catalogo, esq, mid, dir); //Reorganiza os valores do vetor
      }

      return catalogo;
   }

   private static Filme[] intercalar(Filme[] catalogo, int esq, int mid, int dir){
      int nEsq = (mid+1) - esq;
      int nDir = dir - mid;

      //Cria os vetores da esquerda e da direita para futura intercalação
      Filme[] catEsq = new Filme[nEsq + 1];
      Filme[] catDir = new Filme[nDir + 1];

      //Seta o orçamento dos filmes sentinelas para paragem de intercalação de cada vetor
      catEsq[nEsq] = new Filme();
      catDir[nDir] = new Filme();
      catEsq[nEsq].setOrcamento(Float.MAX_VALUE);
      catDir[nDir].setOrcamento(Float.MAX_VALUE);

      //Joga os valores no vetor da esquerda
      int iEsq, iDir, i;
      for (iEsq = 0; iEsq < nEsq; iEsq++) {
         catEsq[iEsq] = catalogo[esq + iEsq];
      }

      //Joga os valores no vetor da direita
      for (iDir = 0; iDir < nDir; iDir++) {
         catDir[iDir] = catalogo[(mid+1) + iDir];
      }

      //Joga os valores de ambos dos vetores, na ordem crescente, no vetor real
      //utilizando o valor sentinela (Float.MAX_VALUE)
      for(iDir = iEsq = 0, i = esq; i <= dir; i++){
         catalogo[i] = (catEsq[iEsq].getOrcamento() <= catDir[iDir].getOrcamento()) ? 
                        catEsq[iEsq++] : catDir[iDir++];
      }
      return catalogo;
   }
   ...
   public static void main(String[] args) throws Exception {
      ...
      catalogo = mergeSort(catalogo); // Chama a função de ordenação
      ...
      RandomAccessFile matriculaSelecao = new RandomAccessFile("matricula_quicksort.txt", "rw");
