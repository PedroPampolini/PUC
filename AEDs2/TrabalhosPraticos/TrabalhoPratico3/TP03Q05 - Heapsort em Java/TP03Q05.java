...
Data de Criação: 16/05/2022      Última atualização: 16/05/2022
Objetivo: Recebe uma lista de nome de arquivos, abre e realiza a leitura dos mesmos salvando os valores eo objetos da
classe Filme. Então ordena os filmes a partir do atributo de generos utilizando o algorítmo heap sort
*/
...
public class TP03Q05 {
   ...
   // Recebe 2 filmes, se o atributo tal de f1 for maior, retorna > 0, se de f2 for
   // maior, retorna < 0
   public static int cmpFilmes(Filme f1, Filme f2) {
      int resp = 0;
      resp = f1.getGenero().compareTo(f2.getGenero());
      if (resp == 0) {
   ...
   public static Filme[] heapSort(Filme[] catalogo) {
      Filme[] heap = new Filme[catalogo.length + 1];   //Cria um vetor para desconsiderar a posição 0
      //Faz o shift dos elementos os atribuindo para o novo vetor
      for (int i = 0; i < catalogo.length; i++) {
         heap[i+1] = catalogo[i];
      }
      //Realiza um swap com todos os itens do vetor para deixá-los em forma de heap
      for (int i = 2; i < heap.length; i++) {
         heap = construirHeap(heap,i);
      }
      
      int tam = catalogo.length; //Atribui a posição máxima para reconstruir o heap]
      //Enquanto essa posição for maior que 1, significa que ainda há filhos no heap 
      while(tam > 1){
         //Realiza o swap da raiz(maior elemento) com a última posição do vetor, reconstruindo-o do maior para o menor
         swap(heap, 1, tam--);
         reconstruirHeap(heap,tam);
      }

      //Desfaz o shift dos elementos os atribuindo para o vetor recebido como parâmetro
      for (int i = 0; i < catalogo.length; i++) {
         catalogo[i] = heap[i+1];
      }
      return catalogo;
   ...
   public static Filme[] construirHeap(Filme[] heap, int tam){
      //Irá fazer um swap com os elementos de X até 1, e enquanto o filho for maior que o pai
      for (int i = tam; i > 1 && cmpFilmes(heap[i],heap[i/2]) > 0; i/=2) {
   ...
   public static Filme[] reconstruirHeap(Filme[] heap, int tam){
      int i = 1;  //posição da raiz
      //enquando houverem filhos da posição analisada, reconstroi
      while(i <= tam/2){
         int filho = getMaiorFilho(heap,i,tam); //Pega o maior filho da posição
         count++;
         //Compara se o pai é menor que o filho, se for, realiza o swap e passa para a proxima posição, o filho
         if (cmpFilmes(heap[i],heap[filho]) < 0) {
            heap = swap(heap, i, filho);
            i = filho;
         }
         else{
            i = tam; //Caso o pai seja maior que o filho, não ha necessidade de continuar a verificação, entao sai do loop
         }
      }
      return heap;
   }

   public static int getMaiorFilho(Filme[] vet,int pai,int tam){
      int filho;
      //Caso o filho da esquerda esteja na posição maxima, significa que seu irmão não existe
      //Caso ele exista, compara os 2 irmãos e retorna o maior
      if (pai*2 == tam || cmpFilmes(vet[pai*2], vet[pai*2 + 1]) > 0) {
         filho = pai*2;
      }
      else{
         filho = pai*2 + 1;
   ...
   public static Filme[] swap(Filme[] vet, int pos1, int pos2){
      movim +=3;
      Filme tmp = vet[pos1];
      vet[pos1] = vet[pos2];
      vet[pos2] = tmp;
   ...
   public static void main(String[] args) throws Exception {
      ...
      // Cria um vetor dos filmes com o tamanhp igual ao número de nomes de arquivos
      // lidos
      ...
      catalogo = heapSort(catalogo); // Chama a função de ordenação
      ...
      RandomAccessFile matriculaSelecao = new RandomAccessFile("matricula_heapsort.txt", "rw");
