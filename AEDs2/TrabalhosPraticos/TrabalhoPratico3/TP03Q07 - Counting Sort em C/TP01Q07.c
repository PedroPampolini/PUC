...
int cmpFilme(Filme f1, Filme f2){
    int resp = 0;
    resp = strcmp(f1.idiomaOriginal,f2.idiomaOriginal);
...
int getMaiorDuracao(ArrayList* catalogo){
    int maiorDuracao = 0;
    for (int i = 0; i < catalogo->len; i++)
    {
        if (catalogo->lista[i].duracao > maiorDuracao)
        {
            maiorDuracao = catalogo->lista[i].duracao;
        }
...
ArrayList* countingSort(ArrayList* catalogo){
    int tamCount = getMaiorDuracao(catalogo) + 1;
    int count[tamCount];
    ArrayList* ordenado = (ArrayList*)malloc(sizeof(ArrayList));

    //Inicializa o vetor de contagem de filme
    for (int i = 0; i < tamCount; count[i] = 0, i++);

    //Faz a contagem dos elementos
    for (int i = 0; i < catalogo->len;count[catalogo->lista[i].duracao]++, i++);

    //Faz a soma de x com x-1
    for (int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordena os elementos
    for (int i = catalogo->len-1; i >= 0;i--)
    {
        ordenado->lista[count[catalogo->lista[i].duracao] -1] = catalogo->lista[i];
        count[catalogo->lista[i].duracao]--;
    }
...
int main(){
    ...
    catalogo = *countingSort(&catalogo);
