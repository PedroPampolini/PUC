...
classe Filme. Então ordena os filmes a partir do atributo de título original utilizando o algorítmo selection sort
*/
...
public class TP03Q01 {
    ...
    public static Filme[] selectionSort(Filme[] catalogo){
        for (int i = 0; i < catalogo.length; i++){
            for (int j = i+1; j < catalogo.length; j++) {   //Seleciona o menor elemento 
                //Lê o valor da string dos títulos para melhor organização do código
                String f1 = catalogo[i].getTituloOriginal();
                String f2 = catalogo[j].getTituloOriginal();
                count++;

                //Compara se o menor elemento é menor que o analisado no momento
                if (f2.compareTo(f1) < 0) {

                    //Swap de filmes
                    Filme tmp = catalogo[i];
    ...
    private static boolean isFim(String s) {
     ...
     public static void main(String[] args) throws Exception {
        ...
        catalogo = selectionSort(catalogo);     //Chama a função de ordenação
