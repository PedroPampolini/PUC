...
void insertionType(ArrayList* catalogo, int step, int tipo){
    for (int i = (tipo + step); i < catalogo->len; i += step)
    {
        Filme tmp = catalogo->lista[i];
        int j = (i - step);
        while ((j >= 0) && cmpFilme(tmp,catalogo->lista[j]) < 0)
        {
            catalogo->lista[j+step] = catalogo->lista[j];
            j -= step;
        }
        catalogo->lista[j + step] = tmp;
        
    }
    
}

void shellSort(ArrayList* catalogo){
    int step = 1;
    do
    {
        step = (step * 3) + 1;
    } while (step < catalogo->len);
    do
    {
        step /= 3;
        for (int tipo = 0; tipo < step; tipo++)
        {
            insertionType(catalogo,step,tipo);
        }
