...
classe Filme. Então ordena os filmes a partir do atributo de situação utilizando o algorítmo bubble sort
*/
...
public class TP03Q08 {
   ...
   private static Filme[] bubbleSort(Filme[] catalogo){
      
      for (int i = 0; i < catalogo.length - 1; i++) {
         for (int j = i + 1; j < catalogo.length; j++) {
            if(cmpFilmes(catalogo[i], catalogo[j]) > 0){
               swap(catalogo, i, j);
            }
         }
      }
   ...
   public static void main(String[] args) throws Exception {
      ...
      catalogo = bubbleSort(catalogo); // Chama a função de ordenação
