   ...
   public static void main(String[] args) throws Exception {
      ...
      for (int i = 0; i < quantEntradas; i++) {
         ...
         if (comando.charAt(0) == 'I') { // Verifica se o comando é de inserir
            ...
            catalogo.push(f.clone()); // Insere o filme no inicio da lista

         }

         if (comando.charAt(0) == 'R') { // Verifica se o comando é de Remover

            Filme f = catalogo.pop();
...
class Stack{
   ...
   private String toString(Node tmp, int pos){
      String s = new String();
      if(pos == 0){
         s = "[" + pos + "] " + tmp.elemento.toString();
      }
      else{
         s = toString(tmp.prox,pos-1) + "\n[" + pos + "] " + tmp.elemento.toString();
      }
   ...
   public String toString(){
      String s = toString(topo.prox,size-2) + "\n[36] " + topo.elemento.toString();
      return s;      
   }
}
