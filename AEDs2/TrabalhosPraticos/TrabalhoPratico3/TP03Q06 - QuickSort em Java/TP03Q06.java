...
classe Filme. Então ordena os filmes a partir do atributo de situação utilizando o algorítmo quick sort
*/
...
public class TP03Q06 {
   ...
   private static Filme[] quickSort(Filme[] catalogo, int esq, int dir){
      int i, j, pivo = (dir + esq) / 2;
      i = esq; //grava a posição mais a esquerda para poder caminhar para à direita
      j = dir; //grava a posição mais a direita para poder caminhar para à esquerda
      while (i <= j) {
         //enquanto filme na posição i(a esquerda do pivo) for menor que o pivo, vai para o filme 1 posição à direita
         while (cmpFilmes(catalogo[pivo], catalogo[i]) > 0) {
            i++;
         }
         //enquanto filme na posição j(a direita do pivo) for maior que o pivo, vai para o filme 1 posição à esquerda
         while(cmpFilmes(catalogo[pivo], catalogo[j]) < 0){
            j--;
         }
         //Caso as posições selecionadas não tenham se cruzado, faz o swap
         count++;
         if (i <= j) {
            swap(catalogo, i, j);
            i++;
      ...
      count++;
      //Se j não tiver ultrapassado o limite esquerdo, chama a recurssão
      if(esq < j){
         catalogo = quickSort(catalogo,esq,j);
      }
      count++;
      //Se i não tiver ultrapassado o limite direito, chama a recurssão
      if(dir > i){
         catalogo = quickSort(catalogo,i,dir);
      }

      return catalogo;
   }

   //Método que chama e retorna o quicksort recursivo
   public static Filme[] quickSort(Filme[] catalogo) {
      return quickSort(catalogo,0,catalogo.length - 1);
   }
   ...
   public static void main(String[] args) throws Exception {
      ...
      catalogo = quickSort(catalogo); // Chama a função de ordenação
