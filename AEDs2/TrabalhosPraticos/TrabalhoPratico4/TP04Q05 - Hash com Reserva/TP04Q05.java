...
Data de Criação: 18/06/2022      Última atualização: 18/06/2022
...
public class TP04Q05 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Segunda parte, que pesquisa os itens no hash
...
class Hash {
    ...
    private int lastReserva;

    Hash() {
        this(21, 9);
    }

    Hash(int maxHash, int reservaSize) {
        this.maxHash = maxHash;
        this.lastReserva = maxHash;
        tabela = new Filme[maxHash + reservaSize];
    ...
    public void insert(Filme f) throws Exception {
        ...
        if (tabela[pos] != null) {
            if (lastReserva < tabela.length) {
                tabela[lastReserva] = f;
                lastReserva++;
            }
        } else {
            tabela[pos] = f;
    ...
    public int indexOf(String tituloOriginal) {
        ...
        int index = -1;
        if (tabela[pos] != null && tabela[pos].getTituloOriginal().equals(tituloOriginal)) {
            index = pos;
        } else {
            for (int i = maxHash; i < lastReserva; i++) {
                if (tabela[i].getTituloOriginal().equals(tituloOriginal)) {
                    index = i;
                }
            }
        }
    ...
    public void showTable() {
        ...
        for (int i = 0; i < maxHash; i++) {
            System.out.println(i + "\t||\t" + (tabela[i] == null ? "null" : tabela[i].getTituloOriginal()));
        }
        System.out.println("=====areaReserva=====");
        System.out.println("pos\t||\tNome");
        for (int i = maxHash; i < tabela.length; i++) {
            System.out.println(i + "\t||\t" + (tabela[i] == null ? "null" : tabela[i].getTituloOriginal()));
        }
    }
}
