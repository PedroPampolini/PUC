...
class BinaryTree{
   ...
   private Node balancear(Node i){
      if( i != null){
         int fator = Node.getLevel(i.maior) - Node.getLevel(i.menor);
         if (Math.abs(fator) <= 1) {
            i.setLevel();
         }
         //maior mais pesado
         else if(fator >= 2){
            int maiorFilhoFator = Node.getLevel(i.maior.maior) - Node.getLevel(i.maior.menor);
            if (maiorFilhoFator == -1) {
               //rotação à direita do filho
               i.maior = rotacionaDir(i.maior);
            }
            //rotação à esquerda do nó
            i = rotacionaEsq(i);
         }
         //menor mais pesado
         else if(fator <= -2){
            int menorFilhoFator = Node.getLevel(i.menor.maior) - Node.getLevel(i.menor.menor);
            if (menorFilhoFator == 1) {
               //rotação à esquerda do filho
               i.menor = rotacionaEsq(i.menor);
            }
            //rotação à direita do pai
            i = rotacionaDir(i);
         }
      }
      return i;
   ...
   private Node rotacionaDir(Node i){
      Node filho = i.menor;
      i.menor = filho.maior;
      filho.maior = i;
      i.setLevel();
   ...
   private Node rotacionaEsq(Node i){
      Node filho = i.maior;
      i.maior = filho.menor;
      filho.menor = i;
      i.setLevel();
...
class Node{
   ...
   Node(Filme f, Node menor, Node maior, int level){
      this.f = f;
      this.menor = menor;
      this.maior = maior;
      this.level = level;
   ...
   public void setLevel(){
      this.level = 1 + Math.max(getLevel(this.maior),getLevel(this.menor));
   }
}
