...
int main(){
    ...
    for (int i = 0; i < quantComandos; i++)
    {
        ...
        if (comando[0] == 'I')
        {
            ...
            {
                Filme temp;
                getAll(&temp,substring(comando,3,strlen(comando)));
                insertInicio(&catalogo,temp);
            }
            else if (comando[1] == 'F')
            {
                Filme temp;
            ...
            {
                int posicao = (comando[3] - 48) * 10 + (comando[4] - 48);
                Filme temp;
                getAll(&temp,substring(comando,6,strlen(comando)));
                insertPos(&catalogo,temp,posicao);
            }
        ...
        else if (comando[0] == 'R')
        {
            if (comando[1] == 'I')
            {
                Filme f = removeInicio(&catalogo);
                printf("(R) %s\n",f.nome);
            }
            else if (comando[1] == 'F')
            {
                Filme f = removeFim(&catalogo);
                printf("(R) %s\n",f.nome);
            }
            else
            {
                int posicao = (comando[3] - 48) * 10 + (comando[4] - 48);
                Filme f = removePos(&catalogo,posicao);
                printf("(R) %s\n",f.nome);
            }
        }
        else{
