...
public class Exercicio5 {
   ...
   public static void main(String[] args) throws Exception {
      ...
      List catalogo = new List(max * 2);  //Realiza a criação da lista
      ...
      //Realiza a série de operações de inserção
      int quantEntradas = MyIO.readInt();
      String comando = new String();
      for (int i = 0; i < quantEntradas; i++) {
         comando = MyIO.readLine();
         if (comando.charAt(0) == 'I') {  //Verifica se o comando é de inserir

            if (comando.charAt(1) == 'I'){   //Verifica se a inserção é no inicio
               Filme f = new Filme(comando.substring(3));   //Remove o comando de inserção
               f.getAll();    //Recupera a informação do filme
               catalogo.insertInicio(f.clone());   //Insere o filme no inicio da lista
            }
            else if (comando.charAt(1) == 'F'){   //Verifica se a inserção é no inicio
               Filme f = new Filme(comando.substring(3));   //Remove o comando de inserção
               f.getAll();    //Recupera a informação do filme
               catalogo.insertFim(f.clone());   //Insere o filme no fim da lista
            }
            else{    //Se não foi no inicio nem final, será inserir na posição X
               int pos = 0;   //Inteiro que receberá o parse da posição
               pos += (comando.charAt(3) - 48) * 10;  //Calcula a casa das dezenas da posição
               pos += (comando.charAt(4) - 48);    //Calcula a casa das unidades da posição
               Filme f = new Filme(comando.substring(6));   //Remove o comando de inserção
               f.getAll();    //Recupera a informação do filme
               catalogo.insert(f.clone(),pos);   //Insere o filme na lista em X posição 
            }

         }
         
         if (comando.charAt(0) == 'R') {  //Verifica se o comando é de Remover

            if (comando.charAt(1) == 'I'){   //Verifica se a remoção é no inicio
               Filme f = catalogo.removeInicio();
               MyIO.println("(R) " + f.getNome());
            }
            else if (comando.charAt(1) == 'F'){   //Verifica se a remoção é no inicio
               Filme f = catalogo.removeFim();
               MyIO.println("(R) " + f.getNome());
            }
            else{    //Se não foi no inicio nem final, será inserir na posição X
               int pos = 0;   //Inteiro que receberá o parse da posição
      ...
      MyIO.println(catalogo.toString());
   }
}
...
class Filme {
   ...
   public void getAll() throws Exception {
      ...
      aux = reduceSpace(aux);
      int tmpDuracao = 0;
      tmpDuracao = (aux.indexOf("h") != -1) ? parseInt("" + aux.charAt(0)) * 60 : 0; // Realiza o parse do char para inteiro
      if (aux.indexOf("m") != -1) {
         tmpDuracao += (aux.indexOf("h") != -1) ? parseInt(aux.substring(3, 5).replaceAll("m", "")) : parseInt(aux.substring(0, 2).replaceAll("m", "")); 
      }
...
class List{
   ...
   public Filme getPos(int pos){
      return array[pos];
   }

   public void insertFim(Filme f) throws Exception{
      if(len >= array.length){
   ...
   public void insertInicio(Filme f) throws Exception{
      if(len >= array.length){
         throw new Exception("Lista cheia!!");
      }
      for (int i = len; i > 0; i--) {
