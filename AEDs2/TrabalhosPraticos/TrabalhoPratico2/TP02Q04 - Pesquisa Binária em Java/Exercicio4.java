...
classe Filme. Então recebe uma outra série de entradas para realizar uma busca binária para verificar se o filme procurado
se encontra ou não no vetor, se encontram tanto o método iterativo quando o recursivo da busca binária
*/
...
public class Exercicio4 {
   ...
   public static boolean hasMovie(Filme[] vet, String valor, int esq, int dir) {
      int mid = (dir + esq) / 2;    //Calcula o valor do meio
      boolean resp = false;   //Variável que será retornada com o resultado
      //Verifica se os índices das extremidades "colidiram" 
      if (esq <= dir) {
         //Condicional se o filme estiver a direita do meio
         if (valor.compareTo(vet[mid].getNome()) > 0) {
            count += 1;    //Incrementa o contador de contabilização de condições
            esq = mid + 1;    //Atualiza o valor da esquerda para um após o meio
            resp = hasMovie(vet, valor, esq, dir);    //Chama novamente o método com os novos valores
         } 
         //Condicional se o filme estiver a esquerda do meio
         else if (valor.compareTo(vet[mid].getNome()) < 0) {
            count += 2;    //Incrementa o contador de contabilização de condições
            dir = mid - 1;    //Atualiza o valor da direita para um antes do meio
            resp = hasMovie(vet, valor, esq, dir);    //Chama novamente o método com os novos valores
         }
         //Caso o filme analisado for igual ao procurado, seta a variável e não chama mais o método
         else {
            count += 2;    //Incrementa o contador de contabilização de condições
            resp = true;
         }
   ...
   public static boolean hasMovie(Filme[] vet, String valor) {
      return hasMovie(vet, valor, 0, vet.length - 1);    //Chama a função recursiva
   }


   //Método de pesquisa binária iterativa
   public static boolean hasMovieIterative(Filme[] vet, String valor) {
      //Declaração e inicialização das variáveis das extremidades e do meio
      int esq = 0, dir = vet.length - 1, mid = (dir + esq) / 2;
      boolean resp = false;   //Variável que será retornada com o resultado

      while (esq <= dir) {    //Repetirá a busca até os índices das extremidades "colidirem"
         //Condicional se o filme estiver a direita do meio
         if (valor.compareTo(vet[mid].getNome()) > 0) {
            count += 1;    //Incrementa o contador de contabilização de condições
            esq = mid + 1;    //Atualiza o valor da esquerda para um após o meio
            mid = (dir + esq) / 2;  //Recalcula o valor do meio para ficar entra esq e dir
         } 
         //Condicional se o filme estiver a esquerda do meio
         else if (valor.compareTo(vet[mid].getNome()) < 0) {
            count += 2;    //Incrementa o contador de contabilização de condições
         ...
         //Caso o filme analisado for igual ao procurado
         else {
            count += 2;    //Incrementa o contador de contabilização de condições
            resp = true;   //Seta a resposta como true
            esq = dir + 1; //Ativa a condicional de parada
         }
      }
   ...
   public static void main(String[] args) throws Exception {
      ...
      while (!isFim(busca)) {
         ...
         MyIO.println((hasMovie(catalogo, busca)) ? "SIM" : "NAO");
