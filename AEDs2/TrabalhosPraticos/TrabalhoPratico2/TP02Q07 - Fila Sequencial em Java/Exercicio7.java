...
public class Exercicio7 {
   ...
   public static void main(String[] args) throws Exception {
      ...
      CircularQueue catalogo = new CircularQueue(5); // Realiza a criação da lista
      // Insere os elementos no final da lista para manter a ordem
      for (int i = 0; i < max; i++) {
         Filme f = new Filme(listaFilmes[i]);
         f.getAll();
         MyIO.println(catalogo.queue(f.clone()));
      }
      ...
      for (int i = 0; i < quantEntradas; i++) {
         ...
         if (comando.charAt(0) == 'I') { // Verifica se o comando é de inserir
            ...
            MyIO.println(catalogo.queue(f)); // Insere o filme no fim da lista

         }

         else if (comando.charAt(0) == 'R') { // Verifica se o comando é de Remover

            Filme f = catalogo.dequeue();
...
class CircularQueue {
   ...
   CircularQueue(int max) {
      array = new Filme[max + 1];
      size = 0;
   }

   public int queue(Filme f) throws Exception {
      ...
      return (int) Math.round(this.mediaDuracao());
   }

   public Filme dequeue() throws Exception {
      if (first == last) {
   ...
   public String toString() {
      ...
      int i = first;
      while (i != last) {
         s += "[" + i + "] " + array[i].getNome() + " " + array[i].getDuracao() + "\n";
         i = (i + 1) % array.length;
      }
